...
results: `run_all.sage` **44/44**, `vertex_sum_canonical_forms.sage`
**14/14**, `general_canonical_forms.sage` **26/26**, and all seven
...
- **`common.sage`** — the shared foundation everything else in this
  folder builds on:
  - vertex generators for all seven catalog families;
...
- **`simplex.sage`, `hypercube.sage`, `cross_polytope.sage`,
  ...
  `cyclic_polytope.sage`** — one script per family, each covering that
  family's n=1,2,3 instances (or the family-appropriate equivalent — L
  for the associahedron, n=d+3 for cyclic polytopes, the k=2 slice for
  the hypersimplex). Each is runnable on its own (`sage simplex.sage`)
  — it auto-loads `common.sage` if it hasn't been already.
...
- **`*_explorer.ipynb`** — one Jupyter notebook per catalog family (all
  seven now built), each walking through: vertices, canonical form (with
